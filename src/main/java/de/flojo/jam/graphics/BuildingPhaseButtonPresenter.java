...
public class BuildingPhaseButtonPresenter implements IRenderable {
    ...
    public BuildingPhaseButtonPresenter(Screen screen, IProvideContext context, PlayerId ourId) {
        ...
        toolTips.add(new ToolTip<>(giftButton,
                                   () -> "Gift remaining Gold\nDo not use remaining: " + context.getMoneyLeft() + " Gold.",
                                   Color.ORANGE));
        setupGiftButton();
    ...
    private void setupGiftButton() {
        this.giftButton.setLocation(Main.INNER_MARGIN, Main.getHeight() - this.giftButton.getHeight() - 50d);
    ...
    private boolean trapCanBePlacedHere(final Tile t) {
        return (t.getTerrainType() == TerrainTile.EMPTY || t.getTerrainType() == TerrainTile.GRASS_HILL) && context.getFactory().get(
